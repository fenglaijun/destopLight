...
static void ICACHE_FLASH_ATTR exit_fun()
{
	if (g_rsa)
	{
		RSA_free(g_rsa);
...
EdpPacket *ICACHE_FLASH_ATTR PacketEncryptReq(EncryptAlgType type)
{
	BIGNUM *bne = NULL;
	int len = 0;
	int ret = 0;
...
int32 ICACHE_FLASH_ATTR UnpackEncryptResp(EdpPacket *pkg)
{
	uint32 remainlen = 0;
	uint16 key_len = 0;
...
/*************** AES ***************/
static int ICACHE_FLASH_ATTR aes_encrypt(EdpPacket *pkg, int remain_pos)
{
	uint32 in_len = 0;
...
static int ICACHE_FLASH_ATTR aes_decrypt(EdpPacket *pkg, int remain_pos)
{
	size_t in_len = 0;
	unsigned char *in = NULL;
	unsigned char *out = NULL;
...
int ICACHE_FLASH_ATTR SymmEncrypt(EdpPacket *pkg)
{
	int ret = 0;
	uint32 remain_len = 0;
	uint32 remain_pos = 0;
...
int ICACHE_FLASH_ATTR SymmDecrypt(EdpPacket *pkg)
{
	int ret = 0;
	uint32 remain_len = 0;
	uint32 remain_pos = 0;
